...
public class Auto_Blue extends LinearOpMode {
    ...
    private int[] block1CornersBlue = {70, 129, 107, 150};
    private int[] block2CornersBlue = {145, 129, 184, 150};
    private int[] block3CornersBlue = {223, 129, 263, 150};
    private int[] block1CornersRed = {56, 134, 96, 153};
    private int[] block2CornersRed = {135, 134, 174, 153};
    private int[] block3CornersRed = {213, 134, 253, 153};
    ...
    public void runOpMode() throws InterruptedException {
        ...
        skyStoneLocation = findSkyStoneLocation();
     ...
     * @return 0: Pattern A (SkyStone is closest to the wall)
     *         1: Pattern B (SkyStone is in the middle)
     *         2: Pattern C (SkyStone is closest to the bridge)
     */
    private int findSkyStoneLocation() {
        Mat block1, block2, block3;
        Scalar mean1, mean2, mean3, mean1x, mean2x, mean3x;
        int skyStoneLocation = 2;
        if (alliance == Alliance.BLUE) {
...
//            mean3 = Core.mean(block3);
            ...
            if ((mean1x.val[0] > mean2x.val[0]) && (mean1x.val[0] > mean3x.val[0])) skyStoneLocation = 2;
            if ((mean2x.val[0] > mean1x.val[0]) && (mean2x.val[0] > mean3x.val[0])) skyStoneLocation = 1;
            if ((mean3x.val[0] > mean1x.val[0]) && (mean3x.val[0] > mean2x.val[0])) skyStoneLocation = 0;
        }
        else {  // Red alliance
...
//            mean3 = Core.mean(block3);
            ...
            if ((mean1x.val[0] > mean2x.val[0]) && (mean1x.val[0] > mean3x.val[0])) skyStoneLocation = 2;
            if ((mean2x.val[0] > mean1x.val[0]) && (mean2x.val[0] > mean3x.val[0])) skyStoneLocation = 0;
            if ((mean3x.val[0] > mean1x.val[0]) && (mean3x.val[0] > mean2x.val[0])) skyStoneLocation = 1;
        }
